...
_plot_bitmap_16:
        ...
        ext.l   d0                              ; sign extended to long
        ...
        move.w  d0,d1
        lsr.w   #4,d0
        move.w  d0,lv16_word_col(a6)           ; save word column
        andi.w  #15,d1
...
p16_row_loop:
        move.w  lv16_r(a6),d0
        cmp.w   p16_height(a6),d0              ; if r >= height, we are done
        bge     p16_done

        ;   offset = ((row + r) * SCREEN_WORDS_PER_ROW + word_col) * 2
        move.w  p16_row(a6),d1
        add.w   lv16_r(a6),d1                  ; d1 = row + r
        ext.l   d1
        mulu.w  #SCREEN_WORDS_PER_ROW,d1       ; d1 = (row + r) * 40
        move.w  lv16_word_col(a6),d2
        ext.l   d2
        add.l   d2,d1                          ; d1 = word offset into screen
        ...
        ; This discards the off-screen left columns
        move.w  lv16_x_skip(a6),d4
        tst.w   d4
        beq.s   p16_right_clip                 ; no left clip needed
        lsl.w   d4,d3                          ; shift out clipped left bits

p16_right_clip:
        move.w  lv16_visible(a6),d5
        move.w  #16,d6
        sub.w   d4,d6                          ; d6 = 16 - x_skip (unclipped width)
...
p16_do_print:
        move.w  lv16_bit_shift(a6),d6		   ; If bit_shift == 0: word aligned,
        tst.w   d6
        bne.s   p16_unaligned

        or.w    d3,(a0)                        ; aligned: OR source word directly
        bra.s   p16_next_row
...
p16_unaligned:
        ...
        add.w   d6,d7
        cmp.w   #16,d7
        ble.s   p16_next_row                   ; no overflow, skip

        move.w  lv16_word_col(a6),d7
        addq.w  #1,d7
        cmp.w   #SCREEN_WORDS_PER_ROW,d7
        bge.s   p16_next_row                   ; next word is off screen, skip
...
p16_next_row:
        addq.w  #1,lv16_r(a6)                  ; r++
        bra     p16_row_loop                   ; next row
